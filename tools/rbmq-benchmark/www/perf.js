...
function summarise(div, results) {
    ...
    if (mode == 'send') {
    	...
    	div.append('Send rate: ' + '<strong>' + rate + '</strong>msg/s');
    }
    else if (mode == 'recv') {
        rate = Math.round(data['recv-msg-rate']);
    	div.append('Recv rate: ' + '<strong>' + rate + '</strong>msg/s');
    }
    else if (mode == 'send-recv') {
        rate = Math.round((data['send-msg-rate'] + data['recv-msg-rate']) / 2);
    	div.append('Send&Recv rate: ' + '<strong>' + rate + '</strong>msg/s');
    }
    else if (mode == 'latency') {
        rate = Math.round(data['avg-latency']);
    	div.append('Latency: ' + '<strong>' + rate + '</strong>(10^-3)');
    }
}
